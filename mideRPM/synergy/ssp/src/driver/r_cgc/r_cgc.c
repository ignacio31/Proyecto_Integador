...
ssp_err_t R_CGC_ClockStop (cgc_clock_t clock_source)
{
     ...
     */
    CGC_ERROR_RETURN(!(((CGC_CLOCK_PLL == current_clock) || (HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_PLL)))
            && (HW_CGC_PLLClockSourceGet(gp_system_reg) == clock_source)), SSP_ERR_CLOCK_ACTIVE);
