                                     ...
                                     bool                     const restart)
{
    ...
    RIIC_ERROR_RETURN(SSP_SUCCESS == err, SSP_ERR_IN_USE);
...
static ssp_err_t    riic_transfer_open     (riic_instance_ctrl_t * p_ctrl, i2c_cfg_t const * const p_cfg)
{
    ...
    if (NULL != p_cfg->p_transfer_tx)
    {
        ...
        {
            result_rx = p_cfg->p_transfer_rx->p_api->close(p_cfg->p_transfer_rx->p_ctrl);
            RIIC_ERROR_RETURN(SSP_SUCCESS == result_rx, result_rx);
        }
        RIIC_ERROR_RETURN(SSP_SUCCESS == result_tx, result_tx);
    }
    return SSP_SUCCESS;
}
